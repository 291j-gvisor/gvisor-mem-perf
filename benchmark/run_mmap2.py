...
CMDS = ['bin/mmap_private_nofree','bin/mmap_shared_nofree']
...
if __name__ == '__main__':
    ...
    for cmd in CMDS:
        ...
        if WARMUP_TIME == -1: 
            out_file = Path(f'exp2_{mmap_mode}/{runtime}({ITERATIONS})/{cmd_name}.csv')
        else:
            out_file = Path(f'exp2_{mmap_mode}_{WARMUP_TIME}s/{runtime}({ITERATIONS})/{cmd_name}.csv')
